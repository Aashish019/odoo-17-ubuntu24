...
sudo -u postgres psql -c "CREATE USER odoo WITH CREATEDB ENCRYPTED PASSWORD '${ODOO_PG_PASSWORD}';" || true
...
sudo su - odoo -s /bin/bash -c "if [ ! -d /opt/odoo/.git ]; then git clone https://github.com/odoo/odoo --depth 1 --branch 14.0 --single-branch /opt/odoo; else cd /opt/odoo && git pull; fi"
...
echo "✅ Odoo 14 installed successfully on https://${DOMAIN}"
